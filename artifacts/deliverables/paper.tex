...
\begin{itemize}
  ...
  \item \textbf{Empirical comparison to four recent CP methods.} We evaluate SemCP and four recent conformal baselines (ConU, SAFER, LofreeCP, TECP) on TriviaQA and SQuAD with Qwen2.5-7B-Instruct, reporting both marginal and conditional coverage alongside the admissibility rate. The admissibility decomposition makes the gap between sampling-induced and conformal-induced miscoverage transparent.
\end{itemize}
...
\begin{algorithm}[t]
\caption{SemCP --- Semantic Conformal Prediction}
\label{alg:semcp}
\begin{algorithmic}[1]
\REQUIRE Calibration set $\mathcal{D}_{\text{cal}} = \{(x_i, y_i)\}_{i=1}^n$, test prompt $x^*$, LLM $\mathcal{M}$, NLI model $f_{\text{NLI}}$, embedding $\varphi$, coverage level $\alpha$
\STATE \textbf{// Calibration}
\FOR{$i = 1, \ldots, n$}
  \STATE Sample $\{y_i^{(1)}, \ldots, y_i^{(K)}\} \sim \mathcal{M}(\cdot \mid x_i)$
  \STATE Partition samples into clusters via bidirectional NLI under $f_{\text{NLI}}$ (Union--Find)
  \IF{any sample matches a reference of $y_i$}
    \STATE $\tilde{s}_i \gets 1 - \max_{c' \neq c_i^\star} \kappa_\sigma(\bar\varphi_{c_i^\star}, \bar\varphi_{c'})$ \quad\COMMENT{lifted score on the correct cluster}
  \ELSE
    \STATE $\tilde{s}_i \gets +\infty$ \quad\COMMENT{drop from calibration}
  \ENDIF
\ENDFOR
\STATE $\hat{q} \gets \mathrm{Quantile}\!\left(\frac{\lceil(1 - \alpha)(|I|+1)\rceil}{|I|},\; \{\tilde{s}_i : i \in I\}\right)$ where $I = \{i : \tilde{s}_i < \infty\}$

\STATE \textbf{// Prediction}
\STATE Sample $\{y_*^{(1)}, \ldots, y_*^{(K)}\} \sim \mathcal{M}(\cdot \mid x^*)$, partition into clusters $c_1, \ldots, c_m$
\STATE Compute $\tilde{s}(x^*, c_j)$ for each cluster
\RETURN $C(x^*) = \{c_j : \tilde{s}(x^*, c_j) \le \hat{q}\}$
\end{algorithmic}
\end{algorithm}
...
\textbf{Generator.} We use Qwen2.5-7B-Instruct \citep{qwen3technicalreport} as the language model. Qwen2.5-7B-Instruct is a recent open-weight, conversation-tuned LLM that achieves competitive performance on closed-book QA, placing us in a regime where the admissibility probability $p_A$ is large enough that conditional coverage is meaningful. We sample $K = 10$ responses per question with temperature $T = 1.0$, nucleus sampling $p = 0.95$, and a maximum of $64$ new tokens. The chat template includes a short system prompt requesting the shortest factual answer.
...
\textbf{Splits and seeds.} For each dataset we draw $N = 500$ examples from the validation split (with seed 42 for the dataset selection), randomly partition them 50/50 into a calibration set and a test set, and repeat the cal/test partition with three independent random seeds (0, 1, 2) for variance estimation. Results are reported as mean $\pm$ standard deviation across the three seeds, together with bootstrap 95\% confidence intervals on the test fold (1000 resamples). Coverage level is fixed at $\alpha = 0.10$ (target $1 - \alpha = 0.90$).

\textbf{Hardware.} All experiments ran on a single NVIDIA RTX 6000 Ada Generation 48 GB GPU rented from RunPod community cloud (\$0.74/hour). Total wall clock for the full matrix (5 methods $\times$ 3 seeds $\times$ 2 datasets, $N = 500$ each) was approximately TODO\_HOURS hours.
...
\midrule
LLM & Qwen2.5-7B-Instruct (7.6B parameters, BF16) \\
Temperature & $1.0$ \\
...
We compare \textbf{SemCP} against four recent conformal prediction methods designed for LLM generation. All baselines share the same Qwen2.5-7B-Instruct sample pool, NLI partition, and cal/test split, isolating the contribution of the nonconformity score and aggregation rule.
...
\paragraph{TECP \citep{huang2025tecp}.} Token-entropy CP. Uses the mean per-token negative log-likelihood under the same Qwen2.5-7B-Instruct (computed by teacher-forced forward passes at sample time) as the nonconformity score. Strings are mapped to clusters via the same NLI partition.
...
Table~\ref{tab:main_results} presents the complete results across TriviaQA and SQuAD with Qwen2.5-7B-Instruct at $\alpha = 0.10$. We report empirical marginal coverage, conditional coverage on admissible test points (i.e., test instances where at least one of the $K$ samples shares the true meaning class), average set size on the active (non-abstaining) test fold, abstention rate, and the empirical admissibility rate $\hat{p}_A$ on the test fold. All numbers are mean $\pm$ standard deviation across the three cal/test split seeds.

\begin{table}[ht]
\centering
\caption{Main results on TriviaQA and SQuAD with Qwen2.5-7B-Instruct at $\alpha = 0.10$ (target $1-\alpha = 0.90$). Cov$_{\rm marg}$ is empirical marginal coverage; Cov$_{\rm cond}$ is coverage conditional on admissibility; $|C|$ is average set size on non-abstaining points; Abst.\ is abstention rate; $\hat{p}_A$ is empirical admissibility rate. Mean $\pm$ std over 3 cal/test split seeds. Bold = best on metric. \textit{Numbers populated from real experiment runs in artifacts/results/.}}
...
\textbf{Marginal-coverage ceiling.} Empirical marginal coverage is upper-bounded by $\hat{p}_A$. Reading Table~\ref{tab:main_results} from right to left clarifies this: when $\hat{p}_A < 0.90$ no conformal method can reach the nominal level, and the gap $\hat{p}_A - \mathrm{Cov}_{\rm marg}$ measures how much coverage is lost \emph{at the conformal step} rather than at the sampling step. Reporting both decompositions makes the deployment implication transparent: for tasks where Qwen2.5-7B-Instruct's admissibility falls below $1 - \alpha$, practitioners should either invest in a stronger generator, increase $K$, or accept the conditional guarantee as their target.
...
\textbf{Model scale.} Qwen2.5-7B-Instruct is a strong open-weight 8B-class model, but not the largest available. Scaling to 70B+ frontier models would test whether SemCP's set-size advantage persists at frontier admissibility. We expect the relative ranking among methods to be preserved (the conditional-coverage targets are model-independent), but absolute admissibility scales with the generator and would push marginal coverage closer to the conditional value.
...
\textbf{NLI threshold.} The bidirectional NLI binarisation threshold was fixed at $0.5$ and not ablated. The literature on semantic clustering for LLMs treats this as an effectively saturated hyperparameter \citep{kuhn2023semantic}, but a systematic sensitivity analysis is left for future work. The choice of NLI model (DeBERTa-v2-xlarge-MNLI) is also fixed; alternative NLI backbones could change the partition fidelity.
...
\textbf{Sample budget.} $K = 10$ samples per prompt covers the regime where most CP-for-LLM papers operate (ConU, SAFER, LofreeCP, TECP all use $K \in [5, 30]$). High-entropy queries with many valid distinct meanings may still need larger $K$ for the admissibility event to fire reliably; reporting $\hat{p}_A$ explicitly makes this assumption transparent.
...
This paper introduces Semantic Conformal Prediction (SemCP), a framework that lifts conformal prediction from token space into semantic embedding space, producing prediction sets whose elements are meaning equivalence classes rather than surface strings. Our central theoretical contribution is Theorem~\ref{thm:conditional}, which establishes conditional coverage $1 - \alpha - 1/(|I|+1)$ over meaning classes given the admissibility event that the true meaning is among the $K$ samples. The conditioning is necessary: any sampling-restricted CP method has marginal coverage upper-bounded by the admissibility probability $p_A$, so insisting on marginal coverage at the nominal level requires $p_A \ge 1 - \alpha$, a condition external to the calibration procedure.

Empirically, SemCP and four recent CP baselines (ConU, SAFER, LofreeCP, TECP) are evaluated on Qwen2.5-7B-Instruct with TriviaQA and SQuAD at $\alpha = 0.10$. Results are reported with both marginal and conditional coverage, alongside the empirical admissibility rate, making the gap between sampling-induced and conformal-induced miscoverage transparent. We do not claim SemCP dominates every baseline on every metric --- the comparisons in Section~\ref{sec:main_results} document where SemCP's kernel-based score helps and where simpler frequency-based scores (ConU) suffice.

Three contributions follow. First, the corrected theorem unifies SemCP with the conditioning paradigm of ConU and the abstention paradigm of SAFER, situating semantic lifting as a complementary rather than competing axis of progress in CP-for-LLMs. Second, the empirical admissibility-coverage decomposition gives practitioners a diagnostic for deciding whether to invest in conformal calibration at all: when $\hat{p}_A < 1 - \alpha$, no conformal method will deliver marginal coverage, and effort is better spent strengthening the generator. Third, the released codebase implements all five methods behind a single interface, making head-to-head comparisons reproducible.

Future work spans (i) frontier-model evaluation (Llama-3-70B, GPT-4-class) where the admissibility regime is favourable; (ii) per-prompt conditional coverage via adaptive conformal methods; (iii) systematic NLI-threshold and clustering-fidelity ablations; (iv) genuinely open-ended generation tasks (summarisation, dialogue) where the equivalence relation itself is harder to define; and (v) integrating SemCP's kernel scoring with SAFER's risk-bounded abstention to obtain a single method that is meaning-aware, conditioning-aware, and risk-controlled.
...
Answer: [Yes]. Single NVIDIA RTX 6000 Ada Generation 48 GB GPU (community RunPod, \$0.74/hr). Total wall clock: TODO\_HOURS hours for the full matrix (5 methods $\times$ 3 seeds $\times$ 2 datasets, $N=500$ each). Reported in Table~\ref{tab:hyperparams}.
...
Answer: [Yes]. Qwen2.5-7B-Instruct is the language model under evaluation. DeBERTa-v2-xlarge-MNLI is used as the NLI partitioner. all-MiniLM-L6-v2 is used for sentence embeddings. Claude Code (Anthropic) was used as a writing aid; all theorems, proofs, and experimental analysis are the authors' own.
